...
# Adjust for gap between monitors
GAP_BETWEEN_MONITORS=75  # Gap in hundredths of an inch
ADJUSTMENT=$(( GAP_BETWEEN_MONITORS * 96 / 200 ))  # Convert hundredths of an inch to pixels and halve it for each side

for I in ${!NAME[@]}; do
  convert "${1}" -filter Cubic -resize $((MAXX-MINX))x$((MAXY-MINY))\! -crop $(( WIDTH[$I] - ADJUSTMENT * 2 ))x${HEIGHT[$I]}+${OFFSETX[$I]}+${OFFSETY[$I]} +repage "${1%.*}_${I}_${NAME[$I]}.${1##*.}"
done
